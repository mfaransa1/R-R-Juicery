...
export default function ProductDetailSupabase({ slug }: Props) {
    ...
    return (
      <main className="min-h-[70vh] bg-[var(--rr-paper)]">
        <div className="rr-container flex min-h-[70vh] flex-col items-center justify-center text-center">
          ...
          <h1 className="rr-editorial mt-5 text-5xl">Product not found.</h1>
          <Link
            href="/menu"
            className="mt-8 inline-flex items-center gap-2 border border-black bg-black px-6 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-white"
          >
  ...
  const image =
    product.image_path || `/images/products/${product.slug}.jpg`;

  return (
    <main className="bg-[var(--rr-paper)]">
      <section className="border-b border-black/10">
        <div className="grid min-h-[calc(100vh-80px)] lg:grid-cols-2">
          <div className="relative min-h-[55vh] overflow-hidden bg-black/5 lg:min-h-full">
            <img
              src={image}
              alt={product.name}
            ...
            <div className="absolute left-6 top-6 sm:left-8 sm:top-8">
              <Link
                href="/menu"
                className="inline-flex items-center gap-2 border border-white/30 bg-black/35 px-4 py-3 text-xs font-semibold uppercase tracking-[0.14em] text-white backdrop-blur-sm"
              >
                <ArrowLeft className="h-4 w-4" />
                Menu
              </Link>
          ...
          <div className="flex items-center">
            <div className="w-full px-6 py-16 sm:px-10 lg:px-16 lg:py-24">
              ...
              <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-3 border-y border-black/10 py-5">
                ...
                <span className="text-sm text-black/55">{product.size}</span>
              </div>
              ...
              <div className="mt-10 grid gap-3 border-t border-black/10 pt-8 sm:grid-cols-2">
                {[
                  ...
                  >
                    <span className="flex h-7 w-7 items-center justify-center border border-black/15">
